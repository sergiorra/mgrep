...
func discoverDirs(wl *workqueue.WorkQueue, path string) {
	...
	for _, entry := range entries {
		if entry.IsDir() {
			...
			wl.Add(workqueue.NewJob(filepath.Join(path, entry.Name())))
		}
	}
}
		...
		go func() {
			...
			for {
				...
				if workEntry.Path != "" {
					...
					// When the path is empty, this indicates that there are no more jobs available, so we quit
					return
				}
			}
		}()
